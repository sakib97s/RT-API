import {
  Injectable,
  InternalServerErrorException,
  Logger,
} from '@nestjs/common';
import { InjectModel } from '@nestjs/mongoose';
import { Model, Types } from 'mongoose';
import { Vendor } from 'src/pages/vendor/interfaces/vendor.interface';
import { UtilsService } from 'src/shared/utils/utils.service';
import { ResponsePayload } from 'src/interfaces/response-payload.interface';
import { Shop } from 'src/pages/shop/interfaces/shop.interface';
import * as schedule from 'node-schedule';
import { MAX_BANNER_UPLOAD } from '../../config/global-variables';
import { UserNotification } from './interfaces/user-notification.interface';
import {
  AddUserNotificationDto,
  FilterAndPaginationUserNotificationDto,
  GetUserNotificationByIdsDto,
  UpdateUserNotificationDto,
} from './dto/user-notification.dto';
const ObjectId = Types.ObjectId;

@Injectable()
export class UserNotificationService {
  private logger = new Logger(UserNotificationService.name);

  constructor(
    @InjectModel('UserNotification')
    private readonly userNotificationModel: Model<UserNotification>,
    @InjectModel('Shop') private readonly shopModel: Model<Shop>,
    private utilsService: UtilsService,
  ) {
    this.checkExpireEveryday();
  }

  /**
   * addUserNotification()
   * getAllUserNotificationByShop()
   * getUserNotificationById()
   * getAllUserNotifications()
   * getUserNotificationBySlug()
   * getUserNotificationByIds()
   * updateUserNotificationById()
   * updateMultipleUserNotificationById()
   * updateMultipleVendorUserNotificationById()
   * deleteMultipleTrashUserNotification()
   * deleteMultipleUserNotificationByIdByVendor()
   * deleteMultipleUserNotificationById()
   */
  async addUserNotification(
    vendor: Vendor,
    shop: string,
    addUserNotificationDto: AddUserNotificationDto,
  ): Promise<ResponsePayload> {
    try {
      const fShop = await this.shopModel.exists({
        _id: shop,
        'users._id': vendor._id,
      });

      if (!fShop) {
        return {
          success: false,
          message: 'Sorry! you have no access in this shop',
        } as ResponsePayload;
      }

      const totalUserNotification =
        await this.userNotificationModel.countDocuments({
          shop: shop,
        });

      if (totalUserNotification && totalUserNotification > MAX_BANNER_UPLOAD) {
        return {
          success: false,
          message:
            'Sorry! exists your userNotification upload limit with this shop.',
        } as ResponsePayload;
      }

      const finalData = {
        ...addUserNotificationDto,
        ...{
          shop: shop,
        },
      };

      const saveData = await this.userNotificationModel.create(finalData);
      const data = {
        _id: saveData._id,
      };

      return {
        success: true,
        message: 'Success! UserNotification added successfully.',
        data,
      } as ResponsePayload;
    } catch (error) {
      console.log(error);
      throw new InternalServerErrorException(error.message);
    }
  }

  async getAllUserNotificationForUi(shop: string): Promise<ResponsePayload> {
    try {
      const data = await this.userNotificationModel
        .find({
          shop: shop,
          status: 'publish',
        })
        .select('name shortDesc images')
        .sort({ priority: -1 });

      return {
        success: true,
        message: 'Success! Data fetch successfully.',
        data,
      } as ResponsePayload;
    } catch (err) {
      throw new InternalServerErrorException(err.message);
    }
  }

  async getAllUserNotificationByShop(
    shop: string,
    filterUserNotificationDto: FilterAndPaginationUserNotificationDto,
    searchQuery?: string,
  ): Promise<ResponsePayload> {
    try {
      if (!shop) {
        return {
          success: false,
          message: 'Sorry! no data found.',
        } as ResponsePayload;
      }

      // Modify Filter
      const { filter } = filterUserNotificationDto;
      filterUserNotificationDto.filter = { ...filter, ...{ shop: shop } };

      return this.getAllUserNotifications(
        filterUserNotificationDto,
        searchQuery,
      );
    } catch (error) {
      console.log(error);
      throw new InternalServerErrorException(error.message);
    }
  }

  async getUserNotificationById(
    vendor: Vendor,
    shop: string,
    id: string,
    select: string,
  ): Promise<ResponsePayload> {
    try {
      const fShop = await this.shopModel.exists({
        _id: shop,
        'users._id': vendor._id,
      });

      if (!fShop) {
        return {
          success: false,
          message: 'Sorry! you have no access in this shop',
        } as ResponsePayload;
      }

      const data = await this.userNotificationModel
        .findOne({ _id: id, shop: shop })
        .select(select);

      return {
        success: true,
        message: 'Success! Data fetch successfully.',
        data,
      } as ResponsePayload;
    } catch (err) {
      throw new InternalServerErrorException(err.message);
    }
  }

  async getAllUserNotifications(
    filterUserNotificationDto: FilterAndPaginationUserNotificationDto,
    searchQuery?: string,
  ): Promise<ResponsePayload> {
    const { filter } = filterUserNotificationDto;
    const { pagination } = filterUserNotificationDto;
    const { sort } = filterUserNotificationDto;
    const { select } = filterUserNotificationDto;
    const { filterGroup } = filterUserNotificationDto;

    // Aggregate Stages
    const aggregateStages = [];
    const aggregateUserNotificationGroupStages = [];
    const aggregateBrandGroupStages = [];
    const aggregateSubUserNotificationGroupStages = [];

    // Essential Variables
    let mFilter = {};
    let mSort = {};
    let mSelect = {};
    let mPagination = {};

    // Match
    if (filter) {
      if (filter['userNotification._id']) {
        filter['userNotification._id'] = new ObjectId(
          filter['userNotification._id'],
        );
      }

      if (filter['subUserNotification._id']) {
        filter['subUserNotification._id'] = new ObjectId(
          filter['subUserNotification._id'],
        );
      }

      if (filter['brand._id']) {
        filter['brand._id'] = new ObjectId(filter['brand._id']);
      }

      if (filter['tags']) {
        filter['tags'] = new ObjectId(filter['tags']);
      }

      if (filter['shop']) {
        filter['shop'] = new ObjectId(filter['shop']);
      }
    }
    if (filter) {
      mFilter = { ...mFilter, ...filter };
    }
    if (searchQuery) {
      // const mSearchQuery = searchQuery.replace(/[^a-zA-Z0-9 ]/g, '');

      mFilter = {
        $and: [
          mFilter,
          {
            $or: [
              { title: this.utilsService.createRegexFromString(searchQuery) },
              { url: this.utilsService.createRegexFromString(searchQuery) },
              // { name: { $regex: mSearchQuery, $options: 'i' } },
            ],
          },
        ],
      };
    }
    // Sort
    if (sort) {
      mSort = sort;
    } else {
      mSort = { createdAt: -1 };
    }

    // Select
    if (select) {
      mSelect = { ...select };
    } else {
      mSelect = { name: 1 };
    }

    // GROUPING FOR FILTER PRODUCTS
    let groupUserNotification: any;
    let groupBrand: any;
    let groupSubUserNotification: any;

    if (filterGroup && filterGroup.isGroup) {
      if (filterGroup.userNotification) {
        groupUserNotification = {
          $group: {
            _id: { userNotification: '$userNotification._id' },
            name: { $first: '$userNotification.name' },
            slug: { $first: '$userNotification.slug' },
            total: { $sum: 1 },
          },
        };
      }

      if (filterGroup.brand) {
        groupBrand = {
          $group: {
            _id: { brand: '$brand._id' },
            name: { $first: '$brand.name' },
            slug: { $first: '$brand.slug' },
            total: { $sum: 1 },
          },
        };
      }

      if (filterGroup.subUserNotification) {
        groupSubUserNotification = {
          $group: {
            _id: { subUserNotification: '$subUserNotification._id' },
            name: { $first: '$subUserNotification.name' },
            slug: { $first: '$subUserNotification.slug' },
            total: { $sum: 1 },
          },
        };
      }
    }

    // Search A-Z
    if (searchQuery) {
      aggregateStages.push({
        $addFields: {
          sortBySearch: {
            $indexOfCP: ['$name', searchQuery.toLowerCase()],
          },
        },
      });
    }

    // Finalize
    if (Object.keys(mFilter).length) {
      // Main
      aggregateStages.push({ $match: mFilter });

      // UserNotification Groups
      if (groupUserNotification) {
        // aggregateUserNotificationGroupStages.push({ $match: mFilter });
        aggregateUserNotificationGroupStages.push(groupUserNotification);
      }

      // Sub UserNotification Groups
      if (groupSubUserNotification) {
        // aggregateSubUserNotificationGroupStages.push({ $match: mFilter });
        aggregateSubUserNotificationGroupStages.push(groupSubUserNotification);
      }

      // Brand Groups
      if (groupBrand) {
        // aggregateBrandGroupStages.push({ $match: mFilter });
        aggregateBrandGroupStages.push(groupBrand);
      }
    } else {
      if (groupUserNotification) {
        aggregateUserNotificationGroupStages.push(groupUserNotification);
      }
      if (groupSubUserNotification) {
        aggregateSubUserNotificationGroupStages.push(groupSubUserNotification);
      }
      if (groupBrand) {
        aggregateBrandGroupStages.push(groupBrand);
      }
    }

    if (Object.keys(mSort).length) {
      aggregateStages.push({ $sort: mSort });
    }

    if (!pagination) {
      aggregateStages.push({ $project: mSelect });
    }

    // Pagination
    if (pagination) {
      if (Object.keys(mSelect).length) {
        mPagination = {
          $facet: {
            metadata: [{ $count: 'total' }],
            data: [
              {
                $skip: pagination.pageSize * pagination.currentPage,
              } /* IF PAGE START FROM 0 OR (pagination.currentPage - 1) IF PAGE 1*/,
              { $limit: pagination.pageSize },
              { $project: mSelect },
            ],
          },
        };
      } else {
        mPagination = {
          $facet: {
            metadata: [{ $count: 'total' }],
            data: [
              {
                $skip: pagination.pageSize * pagination.currentPage,
              } /* IF PAGE START FROM 0 OR (pagination.currentPage - 1) IF PAGE 1*/,
              { $limit: pagination.pageSize },
            ],
          },
        };
      }

      aggregateStages.push(mPagination);

      aggregateStages.push({
        $project: {
          data: 1,
          count: { $arrayElemAt: ['$metadata.total', 0] },
        },
      });
    }

    try {
      // Main
      const dataAggregates = await this.userNotificationModel.aggregate(
        aggregateStages,
        {
          allowDiskUse: true,
        },
      );

      // GROUP FILTER PRODUCTS DATA
      let userNotificationAggregates: any;
      let subUserNotificationAggregates: any;
      let brandAggregates: any;
      // UserNotification
      if (filterGroup && filterGroup.isGroup && filterGroup.userNotification) {
        userNotificationAggregates = await this.userNotificationModel.aggregate(
          aggregateUserNotificationGroupStages,
          { allowDiskUse: true },
        );
      }

      // Sub UserNotification
      if (
        filterGroup &&
        filterGroup.isGroup &&
        filterGroup.subUserNotification
      ) {
        subUserNotificationAggregates =
          await this.userNotificationModel.aggregate(
            aggregateSubUserNotificationGroupStages,
            { allowDiskUse: true },
          );
      }

      // Brand
      if (filterGroup && filterGroup.isGroup && filterGroup.brand) {
        brandAggregates = await this.userNotificationModel.aggregate(
          aggregateBrandGroupStages,
          { allowDiskUse: true },
        );
      }

      // Main Filter Data
      let allFilterGroups: any;
      if (filterGroup && filterGroup.isGroup) {
        allFilterGroups = {
          categories:
            userNotificationAggregates && userNotificationAggregates.length
              ? userNotificationAggregates
              : [],
          subCategories:
            subUserNotificationAggregates &&
            subUserNotificationAggregates.length
              ? subUserNotificationAggregates
              : [],
          brands:
            brandAggregates && brandAggregates.length ? brandAggregates : [],
        };
      } else {
        allFilterGroups = null;
      }

      if (pagination) {
        if (
          pagination.currentPage < 1 &&
          filter == null &&
          JSON.stringify(sort) == JSON.stringify({ createdAt: -1 })
        ) {
        }

        return {
          ...{ ...dataAggregates[0] },
          ...{
            success: true,
            message: 'Success',
            filterGroup: allFilterGroups,
          },
        } as ResponsePayload;
      } else {
        return {
          data: dataAggregates,
          success: true,
          message: 'Success',
          count: dataAggregates.length,
          filterGroup: allFilterGroups,
        } as ResponsePayload;
      }
    } catch (error) {
      console.log(error);
      throw new InternalServerErrorException();
    }
  }

  async getUserNotificationBySlug(
    shop: string,
    slug: string,
    select: string,
  ): Promise<ResponsePayload> {
    try {
      const data = await this.userNotificationModel
        .findOne({ slug: slug, shop: shop })
        .select(select);

      // Increment view count
      if (data) {
        await this.userNotificationModel.findByIdAndUpdate(data._id, {
          $inc: {
            totalView: 1,
          },
        });
      }

      return {
        success: true,
        message: 'Success! data fetch successfully.',
        data,
      } as ResponsePayload;
    } catch (err) {
      throw new InternalServerErrorException(err.message);
    }
  }

  async getUserNotificationByIds(
    shop: string,
    getUserNotificationByIdsDto: GetUserNotificationByIdsDto,
    select: string,
  ): Promise<ResponsePayload> {
    try {
      const mIds = getUserNotificationByIdsDto.ids.map((m) => new ObjectId(m));
      const data = await this.userNotificationModel
        .find({ _id: mIds, shop: shop })
        .select(select);

      return {
        success: true,
        message: 'Success',
        data,
      } as ResponsePayload;
    } catch (err) {
      throw new InternalServerErrorException(err.message);
    }
  }

  /**
   * updateUserNotificationById
   * updateMultipleUserNotificationById
   */
  async updateUserNotificationById(
    vendor: Vendor,
    shop: string,
    id: string,
    updateUserNotificationDto: UpdateUserNotificationDto,
  ): Promise<ResponsePayload> {
    try {
      // const { name } = updateUserNotificationDto;

      const fShop = await this.shopModel.exists({
        _id: shop,
        'users._id': vendor._id,
      });

      if (!fShop) {
        return {
          success: false,
          message: 'Sorry! you have no access in this shop',
        } as ResponsePayload;
      }

      let finalSlug: string;
      const fData = await this.userNotificationModel.findOne({
        _id: id,
        shop: shop,
      });

      // Check Slug
      // if (fData?.name.trim() !== name.trim()) {
      //   const newSlug = this.utilsService.transformToSlug(name);

      //   const isExists = await this.userNotificationModel.exists({ slug: newSlug });
      //   if (isExists) {
      //     finalSlug = this.utilsService.transformToSlug(name, true);
      //   } else {
      //     finalSlug = newSlug;
      //   }
      // } else {
      //   finalSlug = fData.slug;
      // }

      const finalData = {
        ...updateUserNotificationDto,
      };

      await this.userNotificationModel.findByIdAndUpdate(id, {
        $set: finalData,
      });

      return {
        success: true,
        message: 'Success! data updated successfully.',
      } as ResponsePayload;
    } catch (err) {
      console.log(err);
      throw new InternalServerErrorException();
    }
  }

  async updateMultipleUserNotificationById(
    vendor: Vendor,
    shop: string,
    ids: string[],
    updateUserNotificationDto: UpdateUserNotificationDto,
  ): Promise<ResponsePayload> {
    try {
      const fShop = await this.shopModel.exists({
        _id: shop,
        'users._id': vendor._id,
      });

      if (!fShop) {
        return {
          success: false,
          message: 'Sorry! you have no access in this shop',
        } as ResponsePayload;
      }

      if (ids && ids.length) {
        const mIds = ids.map((m) => new ObjectId(m));

        // Delete No Multiple Action Data
        // if (updateUserNotificationDto.slug) {
        //   delete updateUserNotificationDto.slug;
        // }
        await this.userNotificationModel.updateMany(
          { _id: { $in: mIds } },
          { $set: updateUserNotificationDto },
        );

        return {
          success: true,
          message: 'Success! multiple data updated successfully',
        } as ResponsePayload;
      } else {
        return {
          success: true,
          message: 'Sorry! no id found',
        } as ResponsePayload;
      }
    } catch (err) {
      throw new InternalServerErrorException(err.message);
    }
  }

  async deleteMultipleTrashUserNotification(
    vendor: Vendor,
    shop: string,
    ids: string[],
  ): Promise<ResponsePayload> {
    try {
      const fShop = await this.shopModel.exists({
        _id: shop,
        'users._id': vendor._id,
      });

      if (!fShop) {
        return {
          success: false,
          message: 'Sorry! you have no access in this shop',
        } as ResponsePayload;
      }

      await this.userNotificationModel.deleteMany({
        _id: ids,
        status: 'trash',
      });
      return {
        success: true,
        message: 'Success! UserNotification permanently deleted successfully.',
      } as ResponsePayload;
    } catch (err) {
      throw new InternalServerErrorException(err.message);
    }
  }

  async deleteMultipleUserNotificationByIdByVendor(
    vendor: Vendor,
    shop: string,
    ids: string[],
  ): Promise<ResponsePayload> {
    try {
      const fShop = await this.shopModel.exists({
        _id: shop,
        'users._id': vendor._id,
      });

      if (!fShop) {
        return {
          success: false,
          message: 'Sorry! you have no access in this shop',
        } as ResponsePayload;
      }

      // await this.userNotificationModel.updateMany(
      //   { _id: ids },
      //   {
      //     $set: {
      //       status: 'trash',
      //       deleteDateString: this.utilsService.getDateString(new Date()),
      //     },
      //   },
      // );

      await this.userNotificationModel.deleteMany({ _id: ids });

      return {
        success: true,
        message: 'Success! UserNotification deleted successfully.',
      } as ResponsePayload;
    } catch (err) {
      throw new InternalServerErrorException(err.message);
    }
  }

  async deleteMultipleUserNotificationById(
    ids: string[],
  ): Promise<ResponsePayload> {
    try {
      await this.userNotificationModel.deleteMany({ _id: ids });
      return {
        success: true,
        message: 'Success',
      } as ResponsePayload;
    } catch (err) {
      throw new InternalServerErrorException(err.message);
    }
  }

  async deleteAllTrashByShop(shop: string): Promise<ResponsePayload> {
    try {
      const fShop = await this.shopModel.exists({
        _id: shop,
      });

      if (!fShop) {
        return {
          success: false,
          message: 'Sorry! you have no access in this shop',
        } as ResponsePayload;
      }

      await this.userNotificationModel.deleteMany({
        shop: shop,
        status: 'trash',
      });
      return {
        success: true,
        message: 'Success! order permanently deleted successfully.',
      } as ResponsePayload;
    } catch (err) {
      console.log(err);
      throw new InternalServerErrorException(err.message);
    }
  }

  private async checkExpireEveryday() {
    schedule.scheduleJob('30 3 * * *', async () => {
      await this.checkExpireFromDb();
    });
  }

  private async checkExpireFromDb() {
    try {
      // Calculate the date 10 days ago
      const tenDaysAgo = new Date();
      tenDaysAgo.setDate(tenDaysAgo.getDate() - 10);
      console.log(tenDaysAgo.toISOString().split('T')[0]);
      // Perform deletion of orders with status 'trash' and deleteDateString <= 10 days ago
      await this.userNotificationModel.deleteMany({
        status: 'trash',
        deleteDateString: {
          $lte: tenDaysAgo.toISOString().split('T')[0], // Compare as ISO string for date format matching
        },
      });

      // console.log('Auto-deletion task executed successfully.');
    } catch (err) {
      console.error('Error during auto-deletion:', err);
    }
  }
}
