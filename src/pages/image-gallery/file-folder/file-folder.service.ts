...
export class FileFolderService {
  ...
  async addFileFolder(
    ...
    try {
      const saveData = await this.fileFolderModel.create(addFileFolderDto);
      const data = {
        _id: saveData._id,
      };
      return {
  ...
  async addFileFolderByShop(
    ...
    try {
      ...
      return this.addFileFolder(mData);
    } catch (error) {
      console.log(error);
      throw new InternalServerErrorException(error.message);
    }
  ...
  async getAllFolderByShop(
    ...
    try {
      ...
      filterFileFolderDto.filter = { ...filter, ...{ shop: shop } };

      return this.getAllFileFolders(filterFileFolderDto, searchQuery);
    } catch (error) {
      console.log(error);
      throw new InternalServerErrorException(error.message);
    }
  ...
  async updateFileFolderByIdByShop(
    ...
    try {
      ...
      await this.updateFileFolderById(id, updateFileFolderDto);
